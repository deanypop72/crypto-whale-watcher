...
const binance = (order) => {
  ...
  if(book[symbol] != undefined && u >= book[symbol].lastUId + 1 
    ...
    try {
      if(book[symbol].bids && book[symbol].bids.length > 5)
        outlier = parseFloat(book[symbol].bids[book[symbol].bids.length-1][0]) - (23*sd(book[symbol].bids));
      else
        throw new Error('');
    } catch(e) {
      if(book[symbol].asks && book[symbol].asks.length > 0)
    ...
    try{
      if(book[symbol].asks && book[symbol].asks.length > 5)
        outlier = parseFloat(book[symbol].asks[0][0]) + (23*sd(book[symbol].asks));
      else
        throw new Error('');
    } catch(e) {
      // console.log(book[symbol]);
      if(book[symbol].bids && book[symbol].bids.length > 0)
